...
public class Picture extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		List<Friend> frend=uaciton.getFriendShipIdList(uaciton.getCurrentUser());
		for(Friend f:frend){
		...
		bt_comment.setOnClickListener(new OnClickListener() {			
			@Override
			public void onClick(View v) {
				View view=View.inflate(Picture.this, R.layout.simple_input, null);
				final EditText data=(EditText)view.findViewById(R.id.data);
				Button bt=(Button)view.findViewById(R.id.ok);
				final AlertDialog dio = new AlertDialog.Builder(Picture.this).create();
				dio.setView(view);
				bt.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						uaciton.makeArgument(p.getId(), data.getText().toString());
						list.setAdapter(new CommentAdapter(Picture.this,p.getId()));//调用ImageAdapter.java   
						dio.dismiss();
					}
				});
				dio.show();
